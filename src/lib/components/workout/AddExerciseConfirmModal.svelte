<script lang="ts">
	...
	let { open, exercise, settings, onconfirm, oncancel, loading = false }: Props = $props();
		...
		>
			...
			<div class="flex items-start gap-4 p-5">
				...
				<div class="flex-1 min-w-0">
					<h3 class="font-semibold text-[var(--color-text-primary)] text-lg">Add Exercise</h3>
				</div>
				<button
					type="button"
					onclick={oncancel}
			...
			<!-- Exercise Info -->
			<div class="px-5 pb-4">
				<div class="p-4 rounded-lg bg-[var(--color-accent)]/10 border border-[var(--color-accent)]/30">
					...
					<div class="mt-2 text-sm text-[var(--color-accent)]">
						{settings.sets} sets · {settings.repMin}-{settings.repMax} reps
					</div>
				</div>
				{#if settings.fromDay}
					<p class="text-xs text-center text-[var(--color-text-muted)] mt-3">
						Settings from {settings.fromDay}
					</p>
				{:else}
					<p class="text-xs text-center text-[var(--color-text-muted)] mt-3">
